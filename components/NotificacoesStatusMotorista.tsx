...
/**
 * Card de status das notificações pro motorista (app /motorista).
 * Mesma logica do NotificacoesStatus do gestor, mas o teste envia pro
 * proprio motorista via motorista_id (= auth uid) em vez de empresa_id.
 */
export default function NotificacoesStatusMotorista({ motoristaId }: { motoristaId: string }) {
  ...
  async function checarEstado() {
    ...
    if (Notification.permission === 'default') {
      setEstado('inativo')
      setDetalheTecnico('O navegador ainda não pediu permissão neste dispositivo. Toque em "Ativar" abaixo.')
      return
    }
  ...
  async function enviarTeste() {
    ...
    try {
      const res = await fetch('/api/notificar-agendamento', {
        ...
        body: JSON.stringify({ motorista_id: motoristaId, teste: true }),
      })
      const data = await res.json()
      if (data?.recipients > 0 || data?.id) {
        setMsgTeste('✅ Enviado. Deve chegar em alguns segundos. Se não chegar, veja o aviso acima.')
      } else if (data?.errors) {
        const err = Array.isArray(data.errors) ? data.errors.join(' | ') : JSON.stringify(data.errors)
        setMsgTeste(`⚠️ OneSignal recusou: ${err}`)
      } else {
        setMsgTeste('⚠️ Nenhum dispositivo inscrito. Toque em "Ativar" e tente de novo.')
      }
    } catch (e: any) {
      setMsgTeste(`Erro: ${e?.message || e}`)
    } finally {
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs mt-0.5" style={{ color: '#0F6E56' }}>
              Você vai receber um aviso quando o gestor te atribuir uma corrida.
            </p>
          </div>
          <button
            onClick={enviarTeste}
